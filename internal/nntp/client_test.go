...
func TestConnect_PostingAllowed(t *testing.T) {
	...
	assert.True(t, client.conn.connected, "client.connected")
...
func TestConnect_WithAuthentication(t *testing.T) {
	...
	assert.NoError(t, err, "Connect()")
	assert.True(t, client.conn.connected, "client.connected")
	assert.True(t, client.conn.authenticated, "client.authenticated")
...
func TestClose_Connected(t *testing.T) {
	...
	err = client.Close()
	assert.NoError(t, err, "Close()")
	assert.False(t, client.conn.connected, "client.connected after Close()")
}
