...
// createTestNZB creates an NZB document from nzb.File structs.
...
func TestUsenetFS_Open(t *testing.T) {
	t.Run("NonExistentFile", func(t *testing.T) {
		...
		ufs := NewUsenetFS(context.Background(), &UsenetFSConfig{
			NZB: nzbDoc,
		})
...
func TestUsenetFileInfo(t *testing.T) {
	nzbDoc := createTestNZB(nzb.File{
		Subject: `Test - "test.txt" yEnc (1/1)`,
		Date:    1234567890,
		Segments: []nzb.Segment{
			{MessageId: "msg1@test", Bytes: 1000, Number: 1},
...
func TestUsenetFile_Read(t *testing.T) {
	t.Run("SingleSegment", func(t *testing.T) {
		...
		originalData := generateTestData(100)
		yencEncoded := encodeYenc(originalData, "test.bin", 1, 1, 1, 100, 100)

		server.SetResponse("DATE", "111 20260101000000")
...
func TestUsenetFile_Seek(t *testing.T) {
	setupSeekTest := func(t *testing.T, dataSize int) (*UsenetFile, []byte, func()) {
		...
		server := nntptest.NewServer(t, "200 NNTP Service Ready")
		originalData := generateTestData(dataSize)
		yencEncoded := encodeYenc(originalData, "test.bin", 1, 1, 1, int64(dataSize), int64(dataSize))
	...
	t.Run("SeekSkipsIntermediateSegments", func(t *testing.T) {
		...
		segment1Data := generateTestData(100)
		segment2Data := generateTestData(100)
		segment3Data := generateTestData(100)
		segment4Data := generateTestData(100)

		yencSeg1 := encodeYenc(segment1Data, "test.bin", 1, 4, 1, 100, 400)
		yencSeg2 := encodeYenc(segment2Data, "test.bin", 2, 4, 101, 200, 400)
		yencSeg3 := encodeYenc(segment3Data, "test.bin", 3, 4, 201, 300, 400)
		yencSeg4 := encodeYenc(segment4Data, "test.bin", 4, 4, 301, 400, 400)
		...
		assert.Equal(t, segment3Data[50:60], buf[:n])
		...
		assert.True(t, cmds.HasCommand("BODY <msg2@test>"), "segment 2 should be fetched")
...
func TestUsenetFile_ReadAt(t *testing.T) {
	setup := func(t *testing.T) (*UsenetFile, []byte, *nntptest.Server) {
		...
		segment1Data := generateTestData(50)
		segment2Data := generateTestData(50)
		segment3Data := generateTestData(50)
		segment4Data := generateTestData(50)
		...
		yencSeg1 := encodeYenc(segment1Data, "test.bin", 1, 4, 1, 50, 200)
		yencSeg2 := encodeYenc(segment2Data, "test.bin", 2, 4, 51, 100, 200)
		yencSeg3 := encodeYenc(segment3Data, "test.bin", 3, 4, 101, 150, 200)
		yencSeg4 := encodeYenc(segment4Data, "test.bin", 4, 4, 151, 200, 200)
	...
	t.Run("ReadAtSkipsIntermediateSegments", func(t *testing.T) {
		...
		cmds := server.GetRequestCommands()
		assert.False(t, cmds.HasCommand("BODY <msg1@test>"), "segment 1 should NOT be fetched")
