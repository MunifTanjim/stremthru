...
func (s *FileStream) Read(p []byte) (n int, err error) {
	...
	if s.stream == nil {
		stream, err := s.createSegmentsStream(s.position, s.bufferSegmentCount)
		if err != nil {
			return 0, err
		}
		s.stream = stream
...
func (s *FileStream) ReadAt(p []byte, off int64) (n int, err error) {
	...
	stream, err := s.createSegmentsStream(off, bufferSegmentCount)
	if err != nil {
		return 0, err
	}
	defer stream.Close()
...
func (s *FileStream) createSegmentsStream(startPos int64, bufferSegmentCount int) (*SegmentsStream, error) {
	fileLog.Trace("create segments stream - start", "position", startPos)

	if startPos == 0 {
		return NewSegmentsStream(s.pool, s.file.Segments, s.file.Groups, bufferSegmentCount), nil
	}
	...
	stream := NewSegmentsStream(s.pool, s.file.Segments[result.SegmentIndex:], s.file.Groups, s.bufferSegmentCount)
...
func (s *FileStream) interpolationSearch(targetByte int64) (searchResult, error) {
	...
	fileLog.Trace("search - started", "target_byte", targetByte, "segment_count", segmentCount, "file_size", s.fileSize, "initial_guess", s.estimateSegmentIndex(targetByte))
