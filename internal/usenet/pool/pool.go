...
func (conf *Config) setDefaults() {
	if conf.Log == nil {
		conf.Log = logger.Scoped("nntp/usenet-pool")
	}
...
func (p *Pool) fetchSegment(ctx context.Context, segment *nzb.Segment, groups []string, cache *SegmentCache) (*decodedData, error) {
	messageId := segment.MessageId
	if cache != nil {
		if cachedData, ok := cache.Get(messageId); ok {
			p.Log.Trace("fetch segment - cache hit", "n", segment.Number, "message_id", messageId, "size", len(cachedData.Body))
	...
	result, err, _ := p.fetchGroup.Do(messageId, func() (any, error) {
		...
		for failedAttempts < 3 {
			if len(excludeProviders) > 0 {
				p.Log.Trace("fetch segment - retry", "n", segment.Number, "message_id", messageId, "failed_attempts", failedAttempts, "excluded_providers", len(excludeProviders))
			}
			...
			if err != nil {
				errs = append(errs, err)
				failedAttempts++
				p.Log.Trace("fetch segment - failed to get connection", "error", err, "n", segment.Number, "message_id", messageId)
				continue
			}

			p.Log.Trace("fetch segment - connection acquired", "n", segment.Number, "message_id", messageId, "provider", conn.ProviderId())

			if err := p.ensureConnectionGroup(conn, groups...); err != nil {
				conn.Release()
				errs = append(errs, err)
				failedAttempts++
				p.Log.Trace("fetch segment - failed to ensure group", "error", err, "n", segment.Number, "message_id", messageId, "provider", conn.ProviderId())
				continue
			}
			...
			if err != nil {
				...
				if isArticleNotFoundError(err) {
					...
					p.Log.Trace("fetch segment - article not found", "n", segment.Number, "message_id", messageId, "provider_id", conn.ProviderId())
					continue
				}

				conn.Destroy()
				failedAttempts++
				p.Log.Trace("fetch segment - failed to get body", "error", err, "n", segment.Number, "message_id", messageId, "provider_id", conn.ProviderId())
				continue
			}

			p.Log.Trace("fetch segment - got body", "n", segment.Number, "message_id", messageId, "provider_id", conn.ProviderId())
			...
			if err != nil {
				errs = append(errs, err)
				failedAttempts++
				p.Log.Trace("fetch segment - failed to decode", "error", err, "n", segment.Number, "message_id", messageId)
				continue
			}

			p.Log.Trace("fetch segment - decoded body", "n", segment.Number, "message_id", messageId, "decoded_size", len(data.Body))

			dData := decodedData{data}
			if cache != nil {
				cache.Add(messageId, dData)
			}
