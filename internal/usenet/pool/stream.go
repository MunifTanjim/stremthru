...
type StreamConfig struct {
	Password          string
	SegmentBufferSize int64 // max bytes to buffer (default: 200MB)
}

func (c *StreamConfig) setDefaults() {
	if c.SegmentBufferSize <= 0 {
		c.SegmentBufferSize = 200 * 1024 * 1024 // 200MB
	}
}
...
func (p *Pool) streamArchiveFileInner(archive Archive, archiveType FileType) (*Stream, error) {
	println("streamArchiveFileInner called for", archiveType.String())
	if !archive.IsStreamable() {
...
type StreamSegmentsConfig struct {
	...
	BufferSize int64         // Prefetch buffer size in bytes (default: 100MB)
}
