...
func DetectFileType(fileBytes []byte, filename string) FileType {
	...
	if rarRegex.MatchString(filename) {
		ftLog.Trace("file type - detected", "filename", filename, "type", FileTypeRar, "method", "extension")
		return FileTypeRar
	}

	if sevenZipRegex.MatchString(filename) {
		ftLog.Trace("file type - detected", "filename", filename, "type", FileType7z, "method", "extension")
		return FileType7z
	}
