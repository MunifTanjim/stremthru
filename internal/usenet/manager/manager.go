...
type Manager struct {
	...
	// lockedServers tracks servers that are currently being modified/deleted.
	// New connections cannot be acquired from locked servers.
	lockedServers      map[string]struct{}
	lockedServersMutex sync.RWMutex

	// pendingTimers tracks cleanup timers for old pools
...
// LockServer locks a server for modification, preventing new connections.
// Returns ErrServerLocked if server is already locked or has active connections.
// Must call UnlockServer when done.
...
// IsServerInUse checks if a server has active acquired connections or is locked.
...
// AddServer adds a server from vault to the pool.
func AddServer(providerId string) error {
	...
	if server == nil {
		return nil // Server not found, nothing to add
	}

	pool := globalManager.getPool()
	if pool == nil {
		// Pool not initialized yet; server is saved in vault and will be
		// loaded when GetPool() initializes the pool on first use
		return nil
	}
