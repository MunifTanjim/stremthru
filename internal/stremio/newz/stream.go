...
var indexerSearchCache = cache.NewOtterCache[[]newznab_client.Newz](&cache.CacheConfig{
...
func handleStream(w http.ResponseWriter, r *http.Request) {
	...
	streamBaseUrl := ExtractRequestBaseURL(r).JoinPath("/stremio/newz", eud, "_/strem", id)

	mode := ud.Mode
	includeStream := mode.Stream()
	includeDebrid := mode.Debrid()
	...
	for _, wStream := range wrappedStreams {
		...
		if includeStream {
			...
			if newznab_client.IsNZBCached(wStream.R.Hash) {
				wStream.R.Emoji = "📎"
			}
			...
			steamUrl := streamBaseUrl.JoinPath("st", url.PathEscape(nzbUrl), "/")
		...
		if includeDebrid {
			if storeCode, isCached := isCachedByHash[hash]; isCached && storeCode != "" {
				...
				steamUrl := streamBaseUrl.JoinPath(strings.ToLower(storeCode), url.PathEscape(nzbUrl), "/")
				stream.URL = steamUrl.String()
				cachedStreams = append(cachedStreams, *stream)
				...
				for i := range stores {
					...
					steamUrl := streamBaseUrl.JoinPath(string(storeCode), url.PathEscape(nzbUrl), "/")
					stream.URL = steamUrl.String()
					uncachedStreams = append(uncachedStreams, *stream)
				}
			}
