...
const (
	...
	TorrentInfoCategoryShow    TorrentInfoCategory = "show"
...
func Insert(items []TorrentInfoInsertData, sid string) {
	...
	if strings.Contains(sid, ":") {
		category = TorrentInfoCategoryShow
	}

	hashesBySource := map[TorrentInfoSource][]string{}
	query := get_insert_query(count)
	args := make([]any, 0, 5*count)
	for _, t := range items {
		if _, ok := hashesBySource[t.Source]; !ok {
			hashesBySource[t.Source] = []string{}
		}
		hashesBySource[t.Source] = append(hashesBySource[t.Source], t.Hash)
	...
	go RecordMap(hashesBySource, sid)
}
