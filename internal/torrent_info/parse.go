...
import (
	...
	"github.com/MunifTanjim/stremthru/ptt"
...
func parse(t *TorrentInfo) *TorrentInfo {
	if t.ParserVersion > ptt.VERSION {
		return nil
	}

	r := ptt.Parse(t.TorrentTitle)

	t.ParsedAt = db.Timestamp{Time: time.Now()}
	t.ParserVersion = ptt.VERSION

	t.Audio = r.Audio
	t.BitDepth = r.BitDepth
	t.Channels = r.Channels
	t.Codec = normalize_codec(r.Codec)
	...
	t.Repack = r.Repack
	t.Resolution = normalize_resolution(r.Resolution)
	t.Retail = r.Retail
	t.Seasons = r.Seasons
	t.Site = r.Site
	if r.Size != "" {
