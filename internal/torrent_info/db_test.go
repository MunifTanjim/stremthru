...
func TestUpsertQuery(t *testing.T) {
	expected_query := "INSERT INTO torrent_info AS ti (hash,t_title,size,indexer,src,category,seeders,leechers,private) VALUES " +
		...
		"indexer = CASE WHEN (EXCLUDED.indexer != '' AND ti.indexer != EXCLUDED.indexer) THEN EXCLUDED.indexer ELSE ti.indexer END, " +
		...
		"WHERE EXCLUDED.src = 'dht' OR (EXCLUDED.src != 'ato' AND ti.src NOT IN ('dht','tio','ad','dl','rd')) OR (EXCLUDED.size > 0 AND ti.size < 1) OR (EXCLUDED.indexer != '' AND ti.indexer != EXCLUDED.indexer) OR (EXCLUDED.category != '' AND ti.category = '') OR (EXCLUDED.seeders > 0 AND ti.seeders != EXCLUDED.seeders) OR (EXCLUDED.private = 1 AND ti.private = 0)"
