	...
	ds := util.NewTSVDataset(&util.TSVDatasetConfig[Mapping]{
		...
		ParseRow: func(row []string) (*Mapping, error) {
			m := Mapping{
				...
				Hikka:            parseIntId(row[7]),
				IMDB:             parseStringId(row[8]),
				Kaize:            parseStringId(row[9]),
				KaizeId:          parseIntId(row[10]),
				Kitsu:            parseIntId(row[11]),
				LetterboxdLId:    parseStringId(row[12]),
				LiveChart:        parseIntId(row[15]),
				MyAnimeList:      parseIntId(row[16]),
				Nautiljon:        parseStringId(row[17]),
				NautiljonId:      parseIntId(row[18]),
				Notify:           parseStringId(row[19]),
				Otakotaku:        parseIntId(row[20]),
				Shikimori:        parseIntId(row[21]),
				Shoboi:           parseIntId(row[22]),
				Silveryasha:      parseIntId(row[23]),
				Simkl:            parseIntId(row[24]),
				TMDB:             parseIntId(row[25]),
				TMDBSeasonId:     parseIntId(row[26]),
				TMDBType:         parseStringId(row[27]),
				TVDB:             parseIntId(row[28]),
				TVDBSeasonId:     parseIntId(row[29]),
				Trakt:            parseIntId(row[30]),
				TraktType:        parseStringId(row[35]),
				TraktSeason:      parseIntId(row[32]),
			}
