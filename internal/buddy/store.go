...
func BulkTrackMagnet(s store.Store, tInfos []TorrentInfoInput, tInfoCategory torrent_info.TorrentInfoCategory, storeToken string) {
	...
	if config.HasBuddy {
		...
		start := time.Now()
		if _, err := Buddy.TrackMagnetCache(params); err != nil {
			buddyLog.Error("failed to bulk track magnet cache", "err", core.PackError(err), "hash_count", len(tInfos), "store", s.GetName(), "duration", time.Since(start))
		} else {
	...
	if config.HasPeer && config.PeerAuthToken != "" {
		...
		go func() {
			start := time.Now()
			if _, err := Peer.TrackMagnet(params); err != nil {
				peerLog.Error("failed to bulk track magnet cache", "err", core.PackError(err), "hash_count", len(tInfos), "store", s.GetName(), "duration", time.Since(start))
			} else {
