...
func (m dynamicListMeta) Fetch(client *APIClient) (*List, error) {
	...
	for {
		...
		fetchParams := fetchListParams{Page: page}
		var totalPages, totalResults int
		switch m.MediaType {
		case MediaTypeMovie:
			response := fetchListData[ListItemMovie]{}
			_, err := client.Request("GET", m.Endpoint, fetchParams, &response)
			if err != nil {
				return nil, err
			}
			totalPages, totalResults = response.TotalPages, response.TotalResults
			for i := range response.Results {
				l.Results = append(l.Results, ListItem{
