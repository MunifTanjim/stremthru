...
func TestComplete(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"not remove collection from title", "[Erai-raws] Ninja Collection - 05 [720p][Multiple Subtitle].mkv", true, "Ninja Collection"},
		{"not remove kolekcja from title", "Kolekcja Halloween (1978-2022) [720p] [BRRip] [XviD] [AC3-ELiTE] [Lektor PL]", true, "Halloween"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			result := Parse(tc.ttitle)
			assert.Equal(t, tc.complete, result.Complete)
