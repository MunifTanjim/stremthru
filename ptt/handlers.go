...
var handlers = []handler{
	...
	},
	{
		...
		ValidateMatch: func(input string, match []int) bool {
			return len(strings.Trim(input[match[0]:match[1]], "()[]")) == 4
		},
		Transform: to_year(),
		Remove:    true,
	},
	...
	{
		Field:     "source",
		Pattern:   regexp.MustCompile(`(?i)\bBlu[ .-]*Ray\b(?:.*remux)`),
	...
	},
	{
		...
		Transform: to_value("flac"),
		Remove:    true,
	},
	{
		Field:        "audio",
		Pattern:      regexp.MustCompile(`(?i)\bEAC-?3(?:[. -]?[256]\.[01])?`),
		Transform:    to_value("eac3"),
		Remove:       true,
		KeepMatching: true,
	},
	{
	...
	},
	{
		...
		Transform: to_value("dd5.1"),
		Remove:    true,
	},
	{
		Field:     "audio",
		Pattern:   regexp.MustCompile(`\bQ?AAC(?:[. ]?2[. ]0|x2)?\b`),
		Transform: to_value("aac"),
		Remove:    true,
	},
	{
		Field:     "audioChannels",
	...
	},
	{
		...
		ValidateMatch: validate_and(
			...
			validate_not_at_end(),
			validate_not_match(regexp.MustCompile(`(?i)\[(\d{2,3})](?:(?:\.\w{2,4})$)`)),
		),
		Transform: to_int_array(),
	},
	...
	// });
	{
		...
		Process: func() hProcessor {
			...
			return func(title string, m parseMeta, result map[string]*parseMeta) *parseMeta {
				...
				endIndex := len(title)
				for _, component := range []string{"resolution", "source", "codec", "audio"} {
					if cm, ok := result[component]; ok {
				...
				if len(mIdxs) != 0 {
					...
					if mIdxs[0] == 0 || btReNegBefore.MatchString(mStr) || btReNegAfter.MatchString(mStr) {
						mIdxs, mStr = nil, ""
					} else if len(mIdxs) > 2 {
				...
				if mStr == "" {
					...
					if len(mIdxs) != 0 {
						if mtReNegAfter.MatchString(middleTitle[mIdxs[2]:]) {
							mIdxs, mStr = nil, ""
						} else {
	...
	{
		Field:     "extension",
		Pattern:   regexp.MustCompile(`\.(3g2|3gp|avi|flv|mkv|mk3d|mov|mp2|mp4|m4v|mpe|mpeg|mpg|mpv|webm|wmv|ogm|divx|ts|m2ts|iso|vob|sub|idx|ttxt|txt|smi|srt|ssa|ass|vtt|nfo|html")$`),
		Transform: to_lowercase(),
	},
}
